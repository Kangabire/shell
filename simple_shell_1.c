...
ssize_t input_buf(info_t *info, char **buf, size_t *len)
{
	...
	if (!*len) /* if nothing is left in the buff, fill it */
	{
...
ssize_t get_input(info_t *info)
{
	...
	if (len)	/* we have commands left in the chain buf */
	{
		...
		while (j < len) /* iterate to semicolon or finish */
		{
	...
	*buf_p = buf; /* else not a chain, pass back buffer from _getline() */
	return (r); /* return lengths of Buffer from _getline() */
}
 ...
 * @info: parameter struct
 * @ptr: address of pointers to buffer, pre-allocated / NULL
